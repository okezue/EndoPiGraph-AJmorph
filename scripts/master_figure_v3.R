...
out_pdf<-ifelse(length(args)>=2,args[2],file.path(in_dir,"figures","master_figure_v3.pdf"))
...
class_n<-pair[,.N,by=group]
class_med<-pair[,.(med=median(js)),by=group]
...
bnd_col[,rank:=.I]
hl_col<-c("EPCAM","CD24","KRT8","PIGR","MUC12","CDH1","REG4","CTNNB1","ACTA2","CD3D")
bnd_col[,is_hl:=gene%in%hl_col]
bnd_col[,gene_lab:=ifelse(is_hl|rank<=4,gene,"")]
panC<-ggplot(bnd_col,aes(rank,mean_boundary_frac))+
  geom_point(aes(color=is_hl),size=ifelse(bnd_col$is_hl,3,1.3),alpha=0.85)+
...
bnd_brain[,rank:=.I]
hl_brain<-c("Aqp4","Gfap","Slc17a7","Gad1","Pecam1","Cldn5","Nr2f2","Igf2","Epha4","Calb1","Pvalb","Lamp5","Dcn","Acta2","Nrn1")
...
panD<-ggplot(bnd_brain,aes(rank,mean_boundary_frac))+
  geom_point(aes(color=is_hl),size=ifelse(bnd_brain$is_hl,3,1.3),alpha=0.85)+
  scale_color_manual(values=c("FALSE"="grey60","TRUE"="firebrick"))+
...
panE<-ggplot(null_summary,aes(x=reorder(panel,neglogp),y=neglogp))+
  ...
  coord_flip()+geom_text(aes(label=whole_genome_top),hjust=1.1,size=2.9,color="white",fontface="bold")+
...
if(!is.null(bnd_mibi)){
  ...
  bnd_mibi[,rank:=.I]
  hl_m<-c("CD45","CD40","CD31","CD3","CD68","CD163","GFAP","NeuN","Olig2","FoxP3","Ki67","Arginase1","CD8","Chym_Tryp")
  ...
  panF<-ggplot(bnd_mibi,aes(rank,median_ratio_bnd_over_body))+
    geom_point(aes(color=is_hl),size=ifelse(bnd_mibi$is_hl,3,1.3),alpha=0.85)+
    scale_color_manual(values=c("FALSE"="grey60","TRUE"="firebrick"))+
    geom_hline(yintercept=1,linetype="dashed",color="grey30")+
    geom_text_repel(aes(label=gene_lab),size=2.9,max.overlaps=40)+
    theme_bw()+theme(legend.position="none")+
